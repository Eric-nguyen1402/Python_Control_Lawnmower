...
class RPLidar(object):
    ...
    def __init__(self, port, baudrate, timeout, logger=None):   #None
    ...
    def start_motor(self):
        ...
        # For A2
        self._set_pwm(DEFAULT_MOTOR_PWM)
    ...
    def _read_response(self, dsize):
        ...
        while self._serial.inWaiting() < dsize:
            time.sleep(0.001) # maybe can change the time after response 
    ...
    def iter_measures(self, scan_type='normal', max_buf_meas=500):  #3000
        ...
        if not self.scanning[0]:
            self.start(scan_type) #send command to rplidar sensor
        while True:
            ...
            if self.scanning[2] == 'normal':
                raw = self._read_response(dsize) #receive the response from sensor
                yield _process_scan(raw) #convert the response into angle, distance
            if self.scanning[2] == 'express':
                ...
                yield _process_express_scan(self.express_old_data,
                                            self.express_data.start_angle,
                                            self.express_trame)
    def iter_measures_other(self, scan_type='normal', max_buf_meas=500):  #3000
        '''Iterate over measures. Note that consumer must be fast enough,
        otherwise data will be accumulated inside buffer and consumer will get
        data with increasing lag.
        ...
        while True:
            ...
            if self.scanning[2] == 'express':
                ...
                yield _process_express_scan(self.express_old_data,
                                            self.express_data.start_angle,
                                            self.express_trame)
    def iter_scans(self, scan_type='normal', max_buf_meas=500, min_len=5): #3000
    ...
    def iter_scans_other(self, scan_type='normal', max_buf_meas=500, min_len=5): #3000
        '''Iterate over scans. Note that consumer must be fast enough,
        otherwise data will be accumulated inside buffer and consumer will get
        data with increasing lag.
        ...
        '''
        scan_list = []
        iterator = self.iter_measures_other(scan_type, max_buf_meas)
        for new_scan, quality, angle, distance in iterator:
            if new_scan:
                if len(scan_list) > min_len:
                    yield scan_list
